...
def check_winner():
    ...
    for row in range(3):
        if (board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"] 
            ...
            for column in range(3):
                board[row][column].config(forground=color_yellow, background=color_light_gray)
            game_over = True
            return
    ...
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"]
        ...
        board[0][2].comfig(foreground=color_yellow, background=color_light_gray)
        board[1][1].config(foreground=color_yellow, backgraound=color_light_gray)
        board[2][0].config(foreground=color_yellow, backgrround=color_light_gray)
        game_over = True
        return
...
#window setup
window = Tk() #create the game window
window.title("tic tac toe")
window.resizable(False, False) 

frame = tkinter.Frame(window)
label = tkinter.Label(frame, text=curr_player+"'sturn", font=("consolas", 20), background=color_gray, 
                       foreground="blue")
...
for row in range(3):
    for coloumn in range(3):
        board[row][coloumn] = tkinter.Button(frame, text="", font=("consolas", 50, "bold"),
                                             background=color_gray, foreground=color_blue, width=5, height=2,
                                             command=lambda row=row, column=coloumn: set_tile(row, column))
...
window_x = int((sreen_width/2) - (window_width/2))
